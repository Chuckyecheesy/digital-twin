...
if ([string]::IsNullOrWhiteSpace($ApiUrl)) {
    throw "Missing Terraform output: api_gateway_url. Ensure terraform/outputs.tf defines it, then run terraform apply."
}

if ([string]::IsNullOrWhiteSpace($FrontendBucket)) {
    throw "Missing Terraform output: s3_frontend_bucket. Ensure terraform/outputs.tf defines it, then run terraform apply."
}
